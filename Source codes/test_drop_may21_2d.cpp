...
void runSimulation(float E_bottom, float drop_height, const path& master_dir) {
    try {
        std::cout << "Starting simulation with E_bottom: " << E_bottom << "drop_height: " << drop_height << std::endl;
        ...
        auto mat_type_terrain = DEMSim.LoadMaterial({{"E", 1.1e11}, {"nu", 0.3}, {"CoR", 0.6}, {"mu", 0.65}, {"Crr", 0.01}});
        ...
        float initial_drop_height = drop_height; // Initial high position to avoid overlap
...
int main() {
    ...
    float bottom_boundary_E[] = {5e7, 1e8, 1e10, 2.1e11};  // Example values
    float side_planes_E[] = {1e7};  // Example values
    float drop_heights[] = {0.06, 0.07};  // Example values

    // Create the master directory for all simulation results
    path master_dir = current_path() / "SimulationResults_datamay22";
